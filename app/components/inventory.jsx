...
function Inventory(props){
    ...
    const addItem = async (item,quant,pric)=>{
        ...
        if(docSnap.exists()){
            ...
            await setDoc(docRef, {quantity: a, price: price});
        }else{
        await setDoc(docRef,{quantity: quant, price: pric});
        }
    ...
    const plusOneItem = async (item)=>{
        ...
        if(docSnap.exists()){
        const {quantity} = docSnap.data();
            await setDoc(docRef, {quantity: parseInt(quantity)+1});
        }
        await updateInventory()
    }
